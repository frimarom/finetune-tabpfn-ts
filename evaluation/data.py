...
class Dataset:
    def __init__(
        ...
        if name in CHRONOS_DATASETS_METADATA.keys():
            self.freq = CHRONOS_DATASETS_METADATA[name]["frequency"]
            self.hf_dataset = datasets.load_dataset("autogluon/chronos_datasets", name)["train"].with_format("numpy")
            self.is_chronos = True
            self.gluonts_dataset = to_gluonts_univariate(self.hf_dataset)
            for entry in self.gluonts_dataset:
                entry["freq"] = self.freq
        else:
    ...
    def prediction_length(self) -> int:
        ...
        pred_len = (
            M4_PRED_LENGTH_MAP[freq] if "m4" in self.name
            else (CHRONOS_DATASETS_METADATA[self.name]["prediction_length"]
                  if self.name in CHRONOS_DATASETS_METADATA.keys()
...
if __name__ == "__main__":
    ...
    print(dataset.past_feat_dynamic_real_dim)
    print(dataset.windows)
    print(dataset._min_series_length)
    print(dataset.sum_series_length)
    print(dataset.test_data.input)
    print(next(iter(dataset.test_data.input)))