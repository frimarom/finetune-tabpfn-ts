...
We will start by discussing the background and related work relevant for this thesis.
For this we provide an overview over Time Series Forecasting, Prior fitted networks and finetuning.
\section{Time Series Forecasting}
\label{sec:timeseriesforecasting}
Many processes in industry and also in science can be described by time series data, e.g. stock prices, weather etc.
...
All of these components can be assembled either by adding $x_t = t_t + z_t + s_t + \epsilon_t$ or multiplying $y_t = t_t \cdot z_t \cdot s_t \cdot \epsilon_t$ to create a time series \citep{Dagum2006}.
...
One of the key challenges in time series forecasting is that, in contrast to tabular data, predicted values are not independent of one another but depend on previous observations.
...
Furthermore, the authors provided guidance to split the time series of each dataset into train, validation, and test data, which we will use throughout the whole finetuning process \citep{aksu2024giftevalbenchmarkgeneraltime}.
\subsection{Other Data Sources}
\subsubsection{autogluon/chronos-datasets}
We take additional datasets from the autogluon chronos-datasets dataset collection to have a better basis of datasets we will finetune TabPFN-TS with when we are using multiple datasets \citep{ansari2024chronos}.
...
With this generative process generating large amounts of data that the model will use to train, the key finding of \citet{muller2021transformers} is that this type of architecture does this by approximating the posterior predictive distribution (PPD).
\newline
The authors defined a prior $p(t)$ to generate artificial training tasks where $t$ describes the task the model has to solve/samples data from.
...
The model $q_{\theta}$ predicts the target y, and the Negative Log-Likelihood
\[
\ell_{\theta} = \mathbb{E}{D \cup {x, y} \sim p(D)}[-log{q_{\theta}}(y|x, D)]
\]
, where $D \cup {x,y}$ is a dataset of size $|D|+1$ sampled from $p(D)$, can be used as the loss function, which is minimized using stochastic gradient descent.
...
The prior of TabPFN is implemented with a Structural Causal Model (SCM).
...
$f_i$ is a (nonlinear) deterministic function, $PA\mathcal{G}(i)$ represents the parent nodes of node $i$ in graph $\mathcal{G}f$ and $\epsilon_i$ is noise variable. \newline
...
These periods are independent of time series generation and are only important to give the model increased context over different periodicities.
...
To be more precise, TabPFN uses Structural Causal Models, which are based on graphs to generate this data (see also section PFN).
...
Mostly, time series also have domain-specific cycles, which cannot be identified by calendar features.
...
\newline
Foundation models are often pretrained on large amounts of heterogeneous data to perform best on a vast number of tasks in the specified field.
...
\subsubsection{Definition}
As mentioned above, we define continued pretraining as the method that further trains a foundation model on a domain-specific prior to lead to the potential improvement of it.
\subsubsection{Related Work}
There has not been much work about continuing pre-training a model with a domain-specific prior, but in \citet{kolberg2025tabpfn} we saw that if we continue pre-training TabPFNv2 in a similar modality (tabular data with high feature and low sample count), the model will experience a decrease in performance on non-high-dimensional, low-sample-size data.
