...
Whether it is patients data in healthcare, sales data in finance or every other data which can be stored in an Excel-like schema, tabular data is one of the most common data formats used in a big scale. \citep{van2024tabular}
This type of data has been in the focus by researchers for quite some time but nobody found a way to utilize deep learning for tabular data and algorithms like gradient boost decision trees or random forest were much more accessible.
\newline
But with the release of TabPFN\citep{hollmann2022tabpfn}, a transformer model pre-trained to solve artificially generated classification tasks from a synthetic prior, a new benchmark for tabular inference tasks has been set.
...
\newline
Although there are already models which achieve top rank results in time series forecasting, the impact of finetuning or continued pretraining has not been studied and potential benefits remain uncovered.
\newline
...
\newline
There are many different approaches used in forecasting including Multi-Layer-Perceptron, a Recurrent Neural Network or even Graph Neural Networks. %vague???
...
\end{enumerate}
Chapter 4 will focus on finetuning the model with datasets from the GIFT-EVAL benchmark \citep{aksu2024giftevalbenchmarkgeneraltime} as well as datasets from autogluon/chronos-datasets \citep{ansari2024chronos}.
