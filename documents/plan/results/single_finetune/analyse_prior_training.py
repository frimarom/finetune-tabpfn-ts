...
def load_single_csv(path):
    ...
    if mase_col is None:
        raise ValueError(f"Keine MASE-Spalte in {path} gefunden.")
...
def aggregate_all_csvs(root=SCRIPT_DIR):
    files = sorted(root.rglob("evaluation.csv"))
    if not files:
        raise FileNotFoundError(f"Keine evaluation.csv unter {root} gefunden.")
...
def main():
    ...
    summary.to_csv(SCRIPT_DIR / "mase_checkpoint_summary.csv", index=False)

    make_single_diverging_barplot(
        df=summary,
        value_col="mean_delta_pct",
        output_base=str(SCRIPT_DIR / "mase_mean_delta_plot"),
    ...
    for ckpt in CKPTS:
        ...
        make_single_diverging_barplot(
            ...
            value_col=col,
            output_base=str(SCRIPT_DIR / f"mase_ckpt_{ckpt}_delta_plot"),
            title=f"Relative change in MASE for checkpoint {ckpt}",
        )

    make_combined_barplot(
        df=summary,
        output_base=str(SCRIPT_DIR / "mase_all_combined_plot"),
