...
def _cell_color(val, trained: bool) -> str:
    """Hintergrundfarbe einer Delta-Zelle."""
    if pd.isna(val):
        return "#f0f0f0"
    improved = val < 0
    if trained:
        return "#a8d5a2" if improved else "#f5b895"   # grün / orange
    else:
        return "#aec7e8" if improved else "#f4a6a6"   # hellblau / hellrot


def _row_highlight(trained: bool) -> str:
    """Zeilenfarbe für dataset + num_series Spalten."""
    return "#fffacd" if trained else "white"           # gelb / weiß
...
def generate_table(csv_path: str = DEFAULT_CSV) -> None:
    ...
    for _, row in df.iterrows():
        ...
        cell_colors.append(
            [row_bg, row_bg] + [_cell_color(row.get(c, np.nan), trained) for c in DELTA_COLS]
        )
    ...
    # Legende
    patches = [
        mpatches.Patch(color="#fffacd",  label="Trained on this dataset"),
        mpatches.Patch(color="white",    label="Not trained on"),
        mpatches.Patch(color="#a8d5a2",  label="Improved  (trained)"),
        mpatches.Patch(color="#f5b895",  label="Worsened  (trained)"),
        mpatches.Patch(color="#aec7e8",  label="Improved  (not trained)"),
        mpatches.Patch(color="#f4a6a6",  label="Worsened  (not trained)"),
    ]
    ax.legend(
        handles=patches,
        loc="upper right",
